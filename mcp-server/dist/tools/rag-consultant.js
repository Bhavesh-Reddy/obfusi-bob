 ...
 */
export async function ragConsultant(input) {
    try {
 ...
 */
function buildRAGContext(irData, analysis, userContext) {
    const context = `
 ...
 */
async function queryRAGKnowledgeBase(_context, analysis) {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 500));
    // Determine best technique based on code characteristics
    const hasLoops = analysis.functions.some((fn) => fn.hasLoops);
    const hasConditionals = analysis.functions.some((fn) => fn.hasConditionals);
 ...
 */
function analyzeSecurityRisk(analysis) {
    const complexity = analysis.totalInstructions;
...
// Made with Bob
//# sourceMappingURL=rag-consultant.js.map